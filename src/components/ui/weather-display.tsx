...
const WeatherDisplay: React.FC<WeatherDisplayProps> = ({
  ...
  const getWeatherIcon = (iconCode: string) => {
    ...
    switch (code) {
      ...
      case "10":
        return (
          <CloudArrowDownIcon className="h-12 w-12 text-blue-300 animate-float" />
        );
      case "11":
        return (
          <div className="relative animate-float">
            <CloudIcon className="h-12 w-12 text-gray-100" />
            <div className="absolute inset-0 flex items-center justify-center text-yellow-300 text-xl font-bold">
              ⚡
            </div>
          </div>
        );
      case "13":
        return <HeartIcon className="h-12 w-12 text-blue-100 animate-float" />;
      case "50":
        return (
          <CloudIcon className="h-12 w-12 text-gray-400 opacity-70 animate-float" />
        );
      default:
        return <SunIcon className="h-12 w-12 text-yellow-300 animate-float" />;
    }
